...
function WatchListSendMessageModal(props) {
    ...
    const handleOnTemplateItemClick = (event) => {
        let tmp = currentUser.prename + ' ' + currentUser.lastname + ',\r' + event.target.value;
    ...
    const handleSendMessage = (values) => {
        ...
        if (getLoggedInUser().length > 0) {
            const admin = getLoggedInUser()[0];
            fireBaseBackend.writeMessages(admin, currentUser, messageObj);
        }
    ...
    useEffect(() => {
        if (currentUser) {
            ...
            new APIClient().get('user/' + admin.user_id + '/user_template').then(res => {
                if (res) {
                    let tmp = currentUser.prename + ' ' + currentUser.lastname + ',\r' + res[0].description;
                    form2.setFieldsValue({
                        message: tmp,
                        job_search_profile_id: currentUser.job_search_profile_id,
                        user_id: admin.user_id
                ...
                <Modal title="Write a message for Bäckereifachverkäufer/in Berlin Mitte" open={props.isModalOpen} onCancel={props.handleCancel} width={1000} footer=" ">
                    <div className="row write-msg">
                        <div className="col-md-3 pleft">
                            ...
                            <div className='popup-infor' style={{ "paddingTop": "10%" }}><img src="assets/img/location.svg" alt='' />{currentUser.city} {currentUser.country ? ',' + currentUser.country : ''}</div>
                            <div className='popup-infor'><img src="assets/img/year.svg" alt='' />{currentUser.year_birthday}</div>
                                    ...
                                    <div className="col-md-5">
                                        <div className='row'>
                                            <Form.Item>
                                                ...
                                                <Button type="primary" htmlType='submit' className="btn btn-primary">SEND MESSAGE FOR x CREDIT(S)</Button>
                                            </Form.Item>
                                        </div>
