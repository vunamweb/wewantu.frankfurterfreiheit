...
import JobsTableArchive from '../Tables/JobsTableArchive';
...
class ArchiveJob extends Component {
    ...
    onGetJobSearchProfiles = async () => {
        ...
        await new APIClient().get('user/' + admin.user_id + '/job_search_profiles').then(res => {
            const dataBody = res && res.filter(val => val.is_delete !== 0).map(info => {
                return {
    ...
    render() {
        ...
        return (
            <>
                ...
                <React.Fragment>
                    <JobsTableArchive setActiveTab={this.props.setActiveTab} setsetSearch={this.props.setSearch} jobData={jobData} setTab={setTab} activeTab={this.props.activeTab} />
                </React.Fragment>
            </>
